...
void main_task(intptr_t unused) {
    ...
    ev3_sensor_config(EV3_PORT_3, GYRO_SENSOR);
	...
	ev3_gyro_sensor_reset(EV3_PORT_3);
    while(1) {
    /**
     * PID controller
     */
...
#define black 50
        ...
        {
        	bool_t is_pressed = ev3_touch_sensor_is_pressed(EV3_PORT_2);
        	static bool_t is_initialized = false;
    		start_count++;
        	if (is_initialized == false) {
                ev3_motor_steer(left_motor, right_motor, 0,0);
                ev3_motor_set_power(arm_motor, -1);
            	if ((start_count >= 10) &&(is_pressed == true)) {
            		is_initialized = true;
            		start_count = 0;
